...
let NEXTYEAR=YEAR+1
echo '# VOC '$YEAR 
yadata read pubcit ! filter -t MyOwn "'${VOCTAG}' in tags" ! sort -t MyOwn -k year > voc${YEAR}.myown.yaml
yadata filter -t MyOwn '"cc" in tags' < voc${YEAR}.myown.yaml | 
	yadata render -s -p '%%' -e '{ title: "Karentované (Current Contents) časopisy zahraničné aj domáce" }' kvocka_moje.md
yadata filter -t MyOwn '"scopus" in tags and "cc" not in tags' < voc${YEAR}.myown.yaml | 
	yadata render -s -p '%%' -e '{ title: "Časopisy zahraničné aj domáce evidované v databáze SCOPUS" }' kvocka_moje.md
yadata read pubcit ! filter -t Citation "'${VOCTAG}' in tags" ! exec -t Citation 'is_old = (year == '${PASTYEAR}' and "'${VOCTAG}'" in tags)' ! sort -t Citation -k year > voc${YEAR}.yaml
